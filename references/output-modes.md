...
- 选中的 MusicXML/MXL 和各项验证报告；若状态仍需校正则同时保留原始 OMR 工程、缩略图和日志；
...
默认在 `editable_draft_ready_for_review` 时清除已知中间文件；`editable_draft_needs_correction`、失败和等待选页状态保留全部诊断材料。调试或需要完整过程证据时传入 `-KeepIntermediate $true`。清理结果写入报告的 `cleanup` 字段。
