...
每次创建新输出目录，不覆盖原件。转换成功且无需继续自动校正时，默认删除缩略图、临时 PDF、Audiveris 工程／缓存、验证副本、过程 MSCZ 和过程日志，保留最终 MSCZ、校对 PDF、可选 MIDI、选中的 MusicXML/MXL 与验证报告。失败、等待选页或草稿仍有校正问题时保留中间材料；调试时可显式传入 `-KeepIntermediate $true`。
...
当前共 70 项单元测试和 8 项真实应用回归场景，其中包含编号补偿硬拒绝、中间文件安全清理，以及校正版 MSCZ 自动重导校对 PDF 并重新验收。真实应用集成测试需要另行生成原创测试谱并安装上述依赖；见 [tests](tests/README.md)。仓库不包含用户乐谱、本机配置、应用安装包或历史转换输出。
